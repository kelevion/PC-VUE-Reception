<template>
  <div class="page_root" id="root_demo">
            ...
            <div class="sw_no">
              ...
              <div class="swiper_img">
                ...
                <div class="notice_title">
                  公告栏 <router-link to="notice/list">More</router-link>
                </div>
                <!-- 公告组件 -->
          ...
          <div class="col-12">
            ...
            <div class="overflow-auto" v-else>
              <table
                ...
                role="table"
                aria-busy="false"
                :aria-colcount="fields_goods.length"
                ...
                <tbody>
                  <tr v-for="(o, i) in list_table_goods" :key="i">
                    ...
                    >
                      ...
                      />
                      <span v-else-if="oj.key === 'create_time'">{{
                        o[oj.key] | formatDate
                      }}</span>
          ...
          <div class="col">
            <div class="card_ad">
              ...
              />
               ...
               <!-- 推荐定制商品模块(开始) -->
    <div class="warp" v-if="$check_action('/customized_goods/list', 'get')">
      <!-- 容器 -->
      <div class="container">
        <div class="row">
          <div class="col-12">
            ...
            <div class="diy_recommend">
              ...
              <div class="more_box">
                <router-link to="/customized_goods/list" class="more">
                  <span>More</span>
                </router-link>
              </div>
            </div>
            <!-- 定制商品推荐列表组件 -->
            <list_customized_goods v-if="show_list_customized_goods" :list="list_customized_goods" />
            <div class="overflow-auto" v-else>
              <table
                id="list_diy"
                role="table"
                aria-busy="false"
                :aria-colcount="fields_customized_goods.length"
                class="table b-table table-striped table-hover"
              >
                <thead>
                  <tr>
                    <th v-for="(o,i) in fields_customized_goods" :key="i">
                      {{ o.label }}
                    </th>
                  </tr>
                </thead>
                <tbody>
                  <tr
                    v-for="(o, i) in list_table_customized_goods"
                    :key="i"
                    @click="to_details('customized_goods', o, 'customized_goods_id')"
                  >
                    <td v-for="(oj,n) in fields_customized_goods">
                      <img
                        v-if="oj.type && oj.type == '图片'"
                        :src="$fullUrl(o[oj.key])"
                        alt=""
    ...
    <!-- 推荐定制商品模块(结束) -->
              <!-- 推荐网站公告模块(开始) -->
    <div class="warp" v-if="$check_action('/website_announcement/list', 'get')">
      <!-- 容器 -->
      <div class="container">
        <div class="row">
          <div class="col-12">
            ...
            <div class="overflow-auto" v-else>
              <table
                ...
                role="table"
                aria-busy="false"
                :aria-colcount="fields_website_announcement.length"
                class="table b-table table-striped table-hover"
              >
                <thead>
                  <tr>
                ...
                <tbody>
                  <tr
                    v-for="(o, i) in list_table_website_announcement"
                    :key="i"
                    @click="to_details('website_announcement', o, 'website_announcement_id')"
                  >
    ...
    <!-- 推荐网站公告模块(结束) -->
          <!-- 推荐热卖图书模块(开始) -->
    <div class="warp" v-if="$check_action('/hot_books/list', 'get')">
      <!-- 容器 -->
      <div class="container">
        <div class="row">
          <div class="col-12">
            ...
            <div class="diy_recommend">
              ...
              <div class="more_box">
                <router-link to="/hot_books/list" class="more">
                  <span>More</span>
                </router-link>
              </div>
            </div>
            <!-- 热卖图书推荐列表组件 -->
            <list_hot_books v-if="show_list_hot_books" :list="list_hot_books" />
            <div class="overflow-auto" v-else>
              <table
                id="list_diy"
                role="table"
                aria-busy="false"
                :aria-colcount="fields_hot_books.length"
                class="table b-table table-striped table-hover"
              >
                <thead>
                  <tr>
                    <th v-for="(o,i) in fields_hot_books" :key="i">
                      {{ o.label }}
                    </th>
                  </tr>
                </thead>
                <tbody>
                  <tr
                    v-for="(o, i) in list_table_hot_books"
                    :key="i"
                    @click="to_details('hot_books', o, 'hot_books_id')"
                  >
                    <td v-for="(oj,n) in fields_hot_books">
                      <img
                        v-if="oj.type && oj.type == '图片'"
                        :src="$fullUrl(o[oj.key])"
                        alt=""
      ...
      <div class="container" :class="{ small_css: isSmall, big_css: !isSmall }">
        ...
        <div class="content" ref="support_content">
          <div v-for="(item, index) in chatList" :key="item + index">
            <div class="item item-center" v-if="item.sendTime">
	...
	export default {
        ...
        "list_type_commodity_type": [],
				...
				show_list_article: true,
								show_list_customized_goods: true,
				list_customized_goods: [],
				vm_arr_customized_goods: [
					"customized_goods_id",
							"trade_name",
							"stock",
							"enclosure",
				...
				fields_customized_goods: [
						...
						type: "日期"
													...
													{
						key: "special_price_or_not",
						label: "是否特价",
						type: "下拉"
					},
									],
								show_list_website_announcement: true,
				list_website_announcement: [],
				vm_arr_website_announcement: [
      ...
      this.get_list_type_commodity_type();
		   ...
		   this.chatList.length && (reallyTime =	nowTimeChuo - this.chatList[this.chatList.length-1]['nowTime'] > spaceTime ?  nowTime : "");
			  ...
			  let	obj = {
				...
				this.sendValue = "";
							...
							// 获取定制商品列表
			get_customized_goods() {
				let url = "~/api/customized_goods/get_list?";
				...
				url = url_get_list;
					...
					if (json.result) {
						this.list_customized_goods = json.result.list;
					}
				})
			},
							// 获取网站公告列表
			get_website_announcement() {
				let url = "~/api/website_announcement/get_list?";
					...
					// 获取热卖图书列表
			get_hot_books() {
				let url = "~/api/hot_books/get_list?";
										url = "~/api/hot_books/get_hits_list?";
						this.$get(url, {
					...
					if (json.result) {
						this.list_hot_books = json.result.list;
					}
				})
			},
		...
		computed: {
						    list_table_hot_books() {
		        var list = this.list_customized_goods;
		        var list_table = [];
		        for (let i = 0; i < list.length; i++) {
		            list_table[i] = {};
		            this.vm_arr_customized_goods.map((o) => {
		                // 第二个中括号是对象的属性
		        ...
		        return list_table;
				    ...
				    list_table_hot_books() {
		        var list = this.list_hot_books;
		        var list_table = [];
		        for (let i = 0; i < list.length; i++) {
		            list_table[i] = {};
		            this.vm_arr_hot_books.map((o) => {
		                // 第二个中括号是对象的属性
		                list_table[i][o] = list[i][o] || "";
		            });
		        }
