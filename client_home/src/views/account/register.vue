			...
			async submit_check(param){
				...
				if(!ret && user_group){
						...
						var form_sub = Object.assign({} ,this.form_sub);
							...
							res = await this.$get("~/api/mall_users/count?", {"user_number": form_sub["user_number"]});
							if(res.result){
								ret = "用户编号已存在!";
							}
						}
							...
							if(user_group === "卖家"){
						var form_sub = Object.assign({} ,this.form_sub);
											if(!ret && !form_sub["seller_account_number"]){
							ret = "卖家账号不能为空!";
						}
						else if(!ret){
							res = await this.$get("~/api/seller/count?", {"seller_account_number": form_sub["seller_account_number"]});
							if(res.result){
								ret = "卖家账号已存在!";
							}
						}
