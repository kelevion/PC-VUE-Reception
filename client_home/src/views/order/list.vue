<template>
  <div class="page_order" id="order_list">
    ...
    color: #fff;
    font-size: 22px;">订单状态</div>
                                <!-- 筛选 -->
                                <div class="bar_filter">
                                    <button type="button" class="btn btn-secondary" v-for="(o, i) in state_list"
                                     :key="i"
                                    @click="state_filter(o)">  {{ o }}</button>
                                </div>
                                <!-- /筛选 -->
                                <div class="bar_filter" style="font-weight: 700;
    color: #fff;
    font-size: 22px;">排序</div>
                                ...
                                >
                                <button type="button" class="btn btn-secondary" @click="sort_order_desc()">价格从高到低</button>
                                    <button type="button" class="btn btn-secondary" @click="sort_order_desc()">价格从低到高</button>
                                    <button type="button" class="btn btn-secondary" v-for="(o, i) in sort_list"
                                    :key="i" @click="sort_order(o)">{{ o.name }}</button>
                                </div>
                                <!-- /排序 -->
                            </div>
